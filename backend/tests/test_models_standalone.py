"""
Standalone Model Testing Script for OpenRouter
...
OPENROUTER_API_KEY = os.getenv("OPENROUTER_API_KEY", "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")
...
def analyze_results(results: List[Dict]):
    ...
    for test_name, test_results in by_test.items():
        ...
        # Calculate quality score: balance of content quality and speed
        for r in successful:
            # Quality score: content length (quality proxy) / response time (speed penalty)
            # Higher content with lower time = better score
            r["quality_score"] = r["content_length"] / max(r["response_time"], 0.1)
        
        # Sort by quality score
        successful.sort(key=lambda x: x["quality_score"], reverse=True)
        
        print(f"\n{'Rank':<5} {'Model':<50} {'Time':<8} {'Tokens':<8} {'Score':<10}")
        print("-" * 80)
        
        for i, r in enumerate(successful[:8], 1):  # Show all results
    ...
    print("\nBased on ACTUAL testing (quality and speed priorities):\n")
    ...
    print(f"  google/gemini-2.5-flash-lite-preview-09-2025")
    ...
    print(f"OPENROUTER_TERTIARY_MODEL=google/gemini-2.5-flash-lite-preview-09-2025")
...
async def main():
    ...
    print(f"Using OpenRouter API: {OPENROUTER_BASE_URL}")
